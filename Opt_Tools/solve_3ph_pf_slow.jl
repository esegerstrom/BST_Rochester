...
@variable(model, Vph_real[ph=1:3,1:n_nodes], start=real(V0_ref[ph]))
@variable(model, Vph_imag[ph=1:3,1:n_nodes], start=imag(V0_ref[ph]))
@variable(model, s_inj_real[1:3,1:n_nodes], start=0)
@variable(model, s_inj_imag[1:3,1:n_nodes], start=0)
@variable(model, Iph_real[1:3,1:n_branches], start=0)
@variable(model, Iph_imag[1:3,1:n_branches], start=0)
@variable(model, l_br_real[1:3,1:3,1:n_branches], start=0)
@variable(model, l_br_imag[1:3,1:3,1:n_branches], start=0)
@variable(model, S_br_real[1:3,1:3,1:n_branches], start=0)
@variable(model, S_br_imag[1:3,1:3,1:n_branches], start=0)

# Complex Variable Expressions
Vph = Vph_real.+im*Vph_imag
s_inj = s_inj_real.+im*s_inj_imag
Iph = Iph_real.+im*Iph_imag
l_br = l_br_real.+im*l_br_imag
S_br = S_br_real.+im*S_br_imag

# Substation Voltage Constraint
@constraint(model, Vph[:,1] .== V0_ref)

# Power Flow Constraints
pb_lhs = zeros(GenericAffExpr{ComplexF64, VariableRef}, 3, n_nodes)
pb_rhs = zeros(GenericAffExpr{ComplexF64, VariableRef}, 3, n_nodes)
for (br_ind,Branch) in enumerate(psm.Branches)
    ...
    @constraint(model, l_br[:,:,br_ind] .== Iph[:,br_ind]*Iph[:,br_ind]')
    @constraint(model, S_br[:,:,br_ind] .== Vph[:,to_node_ind]*Iph[:,br_ind]')
    # Add branch flows to power balance expressions
    pb_lhs[:,to_node_ind] += diag(S_br[:,:,br_ind])
    pb_rhs[:,from_node_ind] += diag(Branch.A_br*S_br[:,:,br_ind]*(Branch.D_br')+Branch.B_br*l_br[:,:,br_ind]*(Branch.D_br'))
end
@constraint(model, pb_lhs + s_inj .== pb_rhs)
...
@constraint(model, s_inj[:,2:end] .== s_gen[:,2:end]-s_load[:,2:end])
